...
void Pathfinding::DrawPath() {
    ...
    for (const auto& pathTile : pathTiles) {
        ...
        Engine::GetInstance().render.get()->DrawTexture(tileX, pathTileWorld.getX() - (float)tileX->w / 4, pathTileWorld.getY() - (float)tileX->h / 4);
    }
...
Vector2D* Pathfinding::PropagateAStar(ASTAR_HEURISTICS heuristic) {
    ...
    if (frontierAStar.size() > 0) {
        ...
        if (frontierTile == playerPosTile) {
            ...
            return new Vector2D(ComputePath(frontierTile.getX(), frontierTile.getY()));
        }
    }
    ...
    if (frontierAStar.size() == 0) return new Vector2D(playerPosTile);
    return nullptr;
}
...
Vector2D Pathfinding::ComputePath(int x, int y)
{
    ...
    if (pathTiles.size() > 2)
    {
        auto pathCopy = pathTiles;
        pathCopy.pop_back();
        pathCopy.pop_back();
        return pathCopy.back();
    }
    if (pathTiles.size() > 1)
    {
        auto pathCopy = pathTiles;
