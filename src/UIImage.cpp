...
bool UIImage::Update(float dt)
{
	...
	case UIElementState::DISABLED:
		Engine::GetInstance().render->DrawTexture(disabledTex, bounds.x, bounds.y, 0, true, 0, 2147483647, 2147483647, useCamera ? 1 : 0);
		break;
	case UIElementState::NORMAL:
		Engine::GetInstance().render->DrawTexture(normalTex, bounds.x, bounds.y, 0, true, 0, 2147483647, 2147483647, useCamera ? 1 : 0);
		break;
	case UIElementState::FOCUSED:
		Engine::GetInstance().render->DrawTexture(focusedTex, bounds.x, bounds.y, 0, true, 0, 2147483647, 2147483647, useCamera ? 1 : 0);
		break;
	case UIElementState::PRESSED:
		Engine::GetInstance().render->DrawTexture(pressedTex, bounds.x, bounds.y, 0, true, 0, 2147483647, 2147483647, useCamera ? 1 : 0);
		break;
	}
