<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.11.2" name="SpriteSheet_Garnik_Infected" tilewidth="32" tileheight="64" tilecount="16" columns="4">
 <image source="SpriteSheet_Garnik_Infected.png" width="128" height="256"/>
 <tile id="0">
  <animation>
   <frame tileid="0" duration="500"/>
   <frame tileid="1" duration="500"/>
   <frame tileid="2" duration="500"/>
   <frame tileid="3" duration="500"/>
  </animation>
 </tile>
 <tile id="4">
  <animation>
   <frame tileid="4" duration="300"/>
   <frame tileid="5" duration="300"/>
   <frame tileid="6" duration="300"/>
   <frame tileid="7" duration="300"/>
  </animation>
 </tile>
 <tile id="8">
  <animation>
   <frame tileid="8" duration="300"/>
   <frame tileid="9" duration="300"/>
   <frame tileid="10" duration="300"/>
   <frame tileid="11" duration="300"/>
  </animation>
 </tile>
 <tile id="12">
  <animation>
   <frame tileid="12" duration="300"/>
   <frame tileid="13" duration="300"/>
   <frame tileid="14" duration="300"/>
   <frame tileid="15" duration="300"/>
  </animation>
 </tile>
</tileset>
